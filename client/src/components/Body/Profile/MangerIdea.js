...
import {
  fetchIdeaOfStaff,
  dispatchIdeaOfStaff,
} from "../../../redux/actions/allIdeaOfStaff";
...
export default function MangerIdea(props) {
  ...
  const handleSubmitUpdate = async (e, id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.post(`/api/update_idea/${id}`, formData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("/api/upload_idea/", formData, {
        headers: {
          Authorization: token,
          "Content-Type":
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(`/api/delete_idea/${id}`, {
            headers: {
              Authorization: token,
              "Content-Type":
                "multipart/form-data; boundary=<calculated when request is sent>",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setModalCate({
            ...modalCate,
            errorCate: error.response.data.msg,
            successCate: "",
          });
        }
      } else if (result.isDenied) {
  ...
  return (
    <div>
      <div style={{ marginTop: "116px" }}>
        ...
        >
          <div className="modal-dialog">
            <div
              ...
              style={{ backgroundColor: "#369598", color: "white" }}
            >
        ...
        {/* content */}
        <div className="ct ">
          <Link to="/profile" className="ct__goProfile">
                ...
                <form style={formSearch}>
                    ...
                    <div className="update_detail">
                      ...
                      <div className="form-group">
                        ...
                        <select
                          ...
                          className="col-12"
                          id="category"
                          onChange={(p) => setDepartments(p.target.value)}
                        >
                          {departmentAlls?.map((element, index) => {
                            return (
                              <option
                                key={index}
                                value={element._id}
                                selected={category === element.name}
                              >
                                {element.name}
                              </option>
                            );
                      ...
                      <div className="form-group">
                        ...
                        <p style={formatDateUpdate}>
                          {modalCate.createdAt ? (
                            format(
                              new Date(modalCate?.createdAt),
                              "dd/MM/yyyy HH:mm:ss"
                            )
                          ) : (
                            <p></p>
                  ...
                  <div className="modal-footer">
                    ...
                    <button
                      ...
                      className="btn btn-primary"
                      onClick={(e) => {
                        handleUpdateCate(e, modalCate._id);
                      }}
                    >
                      Save changes
                    </button>
                ...
                <form style={formSearch}>
                    ...
                    <div className="update_detail">
                          ...
                          {categories?.map((element, index) => {
                            return (
                              <option
                                key={index}
                                value={element._id}
                                selected={modal.category === element.name}
                              >
                                {element.name}
                              </option>
                            );
                      ...
                      >
                        ...
                        <input
                          ...
                          id="checkAnonymous"
                          onChange={(e) => {
                            setAnymouserUpdate(e.target.checked);
                          }}
                          checked={anymouserUpdate}
                        />
                      </div>
                      <div className="form-group">
                        {objectMessage ? (
                          <textarea
                            ...
                            style={{
                              height: 50,
                              color: objectMessage.status ? "green" : "red",
                            }}
                            value={objectMessage.message}
                          />
                        ) : (
                          ""
                        )}
                  ...
                  <div className="modal-footer">
                    ...
                    <button
                      ...
                      className="btn btn-primary"
                      onClick={(e) => {
                        handleSubmitUpdate(e, modal._id);
                      }}
                    >
                      Save changes
                    </button>
